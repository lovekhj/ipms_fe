...
import type { PrjDto } from '@/types/project'; // 우리가 정의한 데이터 모양을 가져옵니다.
 ...
 */
export const getPrjList = async (): Promise<PrjDto[]> => {
    try {
        // axios.get을 사용하여 서버의 '/prj/list' 주소로 데이터를 요청합니다.
        // 실제 요청 URL: http://localhost:8080/api/prj/list (Proxy 설정에 따라 달라짐)
        // 요청 주소 확인용 로그
        console.log('API 요청 시작:', `${API_BASE_URL}/prj/list`);
